...
| | 能力 | 你直接得到什么 |
|:---:|:---|:---|
| 🧩 | **规则声明统一** | `validate:"..."` 一处声明，静态校验函数与 `struct tag` 共用同一语义 |
...
生产环境 validator 实例是复用的（预热态），因此用上表 **ValidatorWarm** 对比：

| 场景 | 生成代码 | validator 预热 | 每请求节省 |
| --- | --- | --- | --- |
| CreateUserRequest 合法 | 815 ns/op | 1,612 ns/op | ~797 ns |
| CreateUserRequest 非法（多字段错误） | 835 ns/op（17 allocs） | 2,131 ns/op（29 allocs） | ~1.3 µs，分配约减半 |

按每请求节省换算到单核 CPU：

- **10k RPS**：每秒约省 8 ms CPU（≈0.8% 单核）；
- **100k RPS**：每秒约省 80 ms（≈8% 单核）；非法请求多的接口还省下接近一半的分配，
  GC 压力更小，p99 更稳。

所以对 CPU 打满、SLO 很紧的**热点接口 / 网关**，代码生成值得认真考虑。这也正是
validation-gen 这类**框架无关的独立生成器**存在的意义：热点团队直接拿到收益——
gin 里手动调用一行 `Validate()`，go-zero 里实现 `Validate() error` 接口由 `httpx.Parse`
自动调用，都不需要等框架内置。
